...
//        }
        ...
        if BaseModel.shared.checkTrainningMode(){
            var k = false
            BaseModel.shared.menu.forEach { (configBean) in
                if(configBean.id == DomainConst.KEY_GAS_FORECAST){
                    k = true
                    return
                }
            }
...
//                currentView.showAlert(message: DomainConst.CONTENT00362)
            ...
            //-- BUG0210-SPJ (KhoiVT 2018725) Gasservice - Stock List function
            //++
            case DomainConst.KEY_NEWS_LIST:
                if BaseModel.shared.role_id == "7"{
                    currentView.pushToViewAndClearData(name: "G19F00S01VC")
                }
                else if BaseModel.shared.role_id == "4"{
                    BaseModel.shared.sharedString = "5"
                    BaseModel.shared.sharedTitle = DomainConst.BLANK
                    currentView.pushToViewAndClearData(name: "G19F00S04VC")
                } 
                break
            //--
            //++
            case DomainConst.KEY_GAS_FORECAST:
                currentView.pushToViewAndClearData(name: "G20F01S01VC")
                break
            case DomainConst.TYPE_GAS_SERVICE_REMAIN:
                currentView.pushToViewAndClearData(name: "G20F01S01VC")
                break    
            case DomainConst.KEY_SCAN_QRCODE:
                currentView.pushToViewAndClearData(name: "G21F01S01VC")
                break
            //--
            default:
                currentView.showAlert(message: DomainConst.CONTENT00362)
