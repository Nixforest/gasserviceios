...
//                                      height: CGFloat(_listMaterial.count + _listMaterialOption.count + 1) * GlobalConst.CONFIGURATION_ITEM_HEIGHT)
        ...
        _tblViewGas.frame = CGRect(x: 0, y: offset,
                                   width: GlobalConst.SCREEN_WIDTH,
                                   height: CGFloat(_listMaterial.count + _listMaterialOption.count + _listGasString.count + 1) * GlobalConst.CONFIGURATION_ITEM_HEIGHT)
...
//            btnCancel.isEnabled = (_data.getRecord().show_button_cancel == 1)
    ...
    internal func btnCancelHandler(_ sender: AnyObject) {
        ...
        if let presenter = alert.popoverPresentationController {
            presenter.sourceView = btnCancel
            presenter.sourceRect = btnCancel.bounds
        }
        self.present(alert, animated: true, completion: nil)
    }
...
//        button.backgroundColor          = color
    ...
    private func updateLayout() {
        ...
        _tblViewGas.frame = CGRect(x: 0, y: offset,
                                   width: GlobalConst.SCREEN_WIDTH,
                                   height: CGFloat(_listMaterial.count + _listMaterialOption.count + _listGasString.count ) * GlobalConst.CONFIGURATION_ITEM_HEIGHT)
        _tblViewCylinder.frame = CGRect(x: 0, y: offset,
                                       width: GlobalConst.SCREEN_WIDTH,
                                       height: CGFloat(_listCylinder.count + _listCylinderOption.count + _listMaterialString.count ) * GlobalConst.CONFIGURATION_ITEM_HEIGHT)
        var height = _tblViewGas.frame.height
        //++ BUG0135-SPJ (NguyenPT 20170727) Clear all cylinder
//        if _listMaterial.count < _listCylinder.count {
        if _tblViewCylinder.frame.height > height {
...
//            //-- BUG0137-SPJ (NguyenPT 20170727) Show payback field            
            ...
            case DomainConst.ORDER_INFO_CONTACT_ID:    // Handle make a call
                EventClickCallRequest.request(action: #selector(finishSaveTrack(_:)), view: self, phone: _listInfo[indexPath.row].name.normalizatePhoneString(), type: G07Const.TYPE_BOMOI, obj_id: G05F00S04VC._id)
                let phone = _listInfo[indexPath.row].name.normalizatePhoneString()
                self.makeACall(phone: phone)
                //-- BUG0222-SPJ (KhoiVT 20181004 ) save Track when user call in VIP Order and CustomerOrder
