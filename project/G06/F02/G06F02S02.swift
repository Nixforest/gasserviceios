...
class G06F02S02: StepContent, UITableViewDelegate, UITableViewDataSource,
    UIImagePickerControllerDelegate, ImageTableViewCellDelegate, UINavigationControllerDelegate {
    // MARK: Properties
    /** Selected value */
    ...
    */
    ...
    init(w: CGFloat, h: CGFloat, parent: BaseViewController) {
        ...
        // Table view
        /** Cell define */
        self._tblListImg.register(UINib(nibName: ImageTableViewCell.theClassName, bundle: nil),
                                  forCellReuseIdentifier: ImageTableViewCell.theClassName)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: ImageTableViewCell = tableView.dequeueReusableCell(withIdentifier: ImageTableViewCell.theClassName)
            as! ImageTableViewCell
        cell.indexRow = indexPath.row
        cell.delegate = self
        let aImage :UIImage     = G06F02S02._selectedValue[indexPath.row]
    ...
    public func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        let cell: ImageTableViewCell = tableView.dequeueReusableCell(withIdentifier: ImageTableViewCell.theClassName) as! ImageTableViewCell
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        if let image = info[UIImagePickerControllerEditedImage] as? UIImage {
            G06F02S02._selectedValue.append(image)
    ...
    func removeAtRow(row :Int) {
        ...
        G06F02S02._selectedValue.remove(at: row)
        self._tblListImg.reloadData()
        //NotificationCenter.default.post(name: Notification.Name(rawValue: DomainConst.NOTIFY_NAME_SET_DATA_G01F02), object: nil)
    }
}
